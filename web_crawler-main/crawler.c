...
// Mutex for protecting crawl data structure
pthread_mutex_t crawl_mutex;
...
// -1 on tidy buffer error, and 0 on completoin
...
// crawl_thread function for each thread
void *crawl_thread(void *arg){
    struct ThreadData *data = (struct ThreadData *) arg;
    int ret = process_url(data->url, data->c, NULL);
    if(ret == -3){
        printf("Crawl is full! Max URLs reached.\n");
    }
    else if(ret == -2){
        printf("Curl error: %s\n", data->c->curlErr);
    }
...
void main(void){
	...
	// the following code should be updated to make it multithreaded, but its just kinda what it should mimc kinda
	// initalize crawlers
	...
	// do crawls
	for(int i = 0; i < num; i++){
		int flag = loop(cArr[i]);
		cArr[i]->status = flag;
	}
	// create threads
	pthread_t threads[MAX];
	for(int i = 0; i < MAX; i++){
		pthread_create(&threads[i], NULL, crawl_thread, (void *)c);
	}
	// join threads
	for(int i = 0; i < MAX; i++){
